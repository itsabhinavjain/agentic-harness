...
#
# Responsibilities:
#   - Install Python 3.12 via uv (if not cached)
#   - Install Claude Code (if not present)
#   - Set up .bashrc (PATH, workspace .env sourcing, tool versions)
#
# Everything this script creates is owned by the agent user automatically.
# =============================================================================
set -euo pipefail

# Ensure ~/.local/bin is in PATH (Claude Code installs here)
...
# Added by entrypoint-dev.sh — tool versions
command -v node >/dev/null 2>&1 && echo "  Node: $(node --version)"
command -v uv >/dev/null 2>&1 && echo "  uv: $(uv --version 2>&1 | head -1)"
command -v claude >/dev/null 2>&1 && echo "  Claude Code: $(claude --version 2>&1 | head -1)"
command -v gemini >/dev/null 2>&1 && echo "  Gemini CLI: $(gemini --version 2>&1 | head -1)"
BASHRC_VERSIONS
fi
